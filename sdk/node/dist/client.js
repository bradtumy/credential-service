...
export class CredentialServiceClient {
    constructor({ issuerUrl, verifierUrl, gatewayUrl, fetchImpl } = {}) {
    ...
    async issueVC(request) {
        return this.issue({ ...request, format: 'jwt-vc' });
    }
    async issueSDJWT(request) {
        return this.issue({ ...request, format: 'sd-jwt' });
    }
    async verify(credentialOrRequest) {
        const payload = typeof credentialOrRequest === 'string'
    ...
    async post(baseUrl, path, payload) {
    ...
    async mapError(response) {
        ...
        if (status === 400)
            return new InvalidRequestError(errorText || 'invalid_request', status);
        if (status === 401 || status === 403)
            return new UnauthorizedError(errorText || 'unauthorized', status);
        if (status >= 500)
            return new ServerError(errorText || 'server_error', status);
